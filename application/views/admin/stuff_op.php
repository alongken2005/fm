...
<?php if($kind == 'video'):?>
	...
	<?php if(isset($content['filepic']) && $content['filepic']):?>
		<tr class="tr_icon">
			<th></th>
			<td>
				<img src="<?=base_url('./data/uploads/video_thumb/'.$content['filepic'])?>"/><a href="<?=site_url('admin/stuff/file_del?id='.$content['id'])?>" class="del">删除</a>
			</td>
		</tr>
	<?php endif; ?>
	<?php
		$dir = isset($content['ctime']) ? date('Y/m/', $content['ctime']) : date('Y/m/', time());
		$dir_arr = get_filenames('./data/uploads/video/'.$dir);
	?>
		<tr>
			<th><b>*</b>选择视频：</th>
			<td>
				<?php foreach($dir_arr as $v):?>
				<div class="videoNameList"><input type="radio" name="fname" value="<?=$dir.$v?>" <?=isset($content['filename']) && $content['filename'] == $dir.$v ? 'checked' : ''?>/> <?=$v?></div>
				<?php endforeach;?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th> 标签：</th>
			<td>
				<input type="text" name="tag" value="" class="input2"/>
			</td>
		</tr>
		<tr>
			<th><b>*</b> 创建时间：</th>
...
<?php else:?>
	...
	<?php if(isset($content['filepic']) && $content['filepic']):?>
		<tr class="tr_icon">
			<th></th>
			<td>
				<img src="<?=base_url('./data/uploads/stuff_thumb/'.$content['filepic'])?>"/><a href="<?=site_url('admin/stuff/file_del?id='.$content['id'])?>" class="del">删除</a>
			</td>
		</tr>
	<?php endif; ?>
	<?php
		$dir = isset($content['ctime']) ? date('Y/m/', $content['ctime']) : date('Y/m/', time());
		$dir_arr = get_filenames('./data/uploads/attach/'.$dir);
	?>
		<tr>
			<th>选择附件：</th>
			<td>
				<?php foreach($dir_arr as $v):?>
				<div class="videoNameList"><input type="checkbox" name="attach[]" value="<?=$dir.$v?>" <?=isset($content['filename']) && $content['filename'] == $dir.$v ? 'checked' : ''?>/> <?=$v?></div>
				<?php endforeach;?>
			</td>
		</tr>
		<tr>
